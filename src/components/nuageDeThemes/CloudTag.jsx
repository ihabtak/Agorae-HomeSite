...
class CloudTag extends Component {
  ...
  updateTagCloud(viewpoint){
    ...
    for(var topicID in viewpoint){     
      ...
      tagcloud[topicName].url = MAP21_URL+"#"+USER_API+"topic/"+ viewpoint.id+"/"+ topicID;
  ...
  componentDidMount() {
    ...
    ViewpointIDs.map(async (viewpointID) => {
      await fetch(USER_API+"viewpoint/"+viewpointID, { signal: this.abortController.signal })
  ...
  render() {
    ...
    const myData = [].concat(this.state.tagc)
