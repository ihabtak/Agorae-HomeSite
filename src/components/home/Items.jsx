  ...
  async componentDidMount() {
    ...
    await fetch(USER_API+CdcLinks[0], { signal: this.abortController.signal })
      ...
      this.state.items.map(async (item) => {
        await db.getView('item/'+item["key"][0]+'/'+item["id"])
      .then(data => {
        data = data[item["key"][0]][item["id"]];
        data["itemId"]=item["id"];
        data["corpusId"]=item["key"][0];
        data["url"]=MAP21_URL+"#"+USER_API+"item/"+data["corpusId"]+"/"+data["itemId"];
      ...
      <CardColumns>
            ...
            <Card bg="light" key={idx} style={{flexGrow: 4}}>
              {item["image/video"] === undefined ? 
               ...
               <Card.Img variant="top" src={item["imageUrl"]} />
              :   
               <ReactPlayer controls={true} width="100%" height="80%" url={item["videoUrl"]} />
              }
              <Card.Body>
              ...
              <Card.Title>{item["005 nom de l’élément:"][0]}</Card.Title>
               ...
               :   
               <Card.Subtitle className="mb-2 text-muted">{item["048 organisation:"][0]}</Card.Subtitle>
              }
